...
namespace SGL.Analytics.ExporterClient {
	public class ExporterKeyPairAuthenticator : IExporterAuthenticator {
		...
		public async Task<AuthorizationToken> AuthenticateAsync(string appName, CancellationToken ct = default) {
			...
			var openResponse = await httpClient.SendAsync(openRequest, ct);
			openResponse.EnsureSuccessStatusCode();
			var challengeDto = await openResponse.Content.ReadFromJsonAsync<ExporterKeyAuthChallengeDTO>(jsonOptions, ct);
